...
func (c *AzureFunctionConnectionResolver) Resolve(correlationId string) (*AzureFunctionConnectionParams, error) {
	connection := NewEmptyAzureFunctionConnectionParams()

	connectionParams, err := c.connectionResolver.Resolve(correlationId)
	if err != nil {
		return nil, err
	}
	connection.Append(connectionParams.Value())

	credentialParams, err := c.credentialResolver.Lookup(context.Background(), correlationId)
	if err != nil {
		return nil, err
	}
	...
	// Perform validation
	err = connection.Validate(correlationId)
	if err != nil {
		return nil, err
	}
